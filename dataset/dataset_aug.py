...
class FaceRandOccMaskFaster(data.Dataset):
    ...
    def __init__(self, root_dir, local_rank,
        ...
        super(FaceRandOccMaskFaster, self).__init__()
        self.root_dir = root_dir
        self.local_rank = local_rank
        ...
        # All Unpack to RAM
        self.train_np = np.zeros((self.total, 112, 112, 3), dtype=np.uint8)
        self.mask_out_np = np.zeros((self.total, 112, 112, 3), dtype=np.uint8)
        self.mask_np = np.zeros((self.total, 112, 112, 3), dtype=np.uint8)
        self.train_label_np = np.zeros(self.total, dtype=np.long)
        print('--> start all unpack...')
        # self._all_unpack(self.imgrec, self.train_np, out_label_np=self.train_label_np, read_label=True)
        # self._all_unpack(self.mask_out_rec, self.mask_out_np)
        # self._all_unpack(self.mask_rec, self.mask_np)

        # print('1. unpacking imgrec')
        # self._all_unpack(self.imgrec, self.train_np, out_label_np=self.train_label_np, read_label=True)
        # np.save('train.npy', self.train_np)
        # np.save('label.npy', self.train_label_np)

        print('2. unpacking mask_out')
        self._all_unpack(self.mask_out_rec, self.mask_out_np)
        np.save('mask_out.npy', self.mask_out_np)

        # print('3. unpacking mask')
        # self._all_unpack(self.mask_rec, self.mask_np)
        # np.save('mask.npy', self.mask_np)

        print('--> finished unpack')

    def _all_unpack(self, imgrec, out_img_np, lo=-1, hi=-1, out_label_np=None, read_label=False):
        from tqdm import tqdm
        for index in tqdm(range(self.total)):
            idx = self.imgidx[index]
            ...
            if read_label:
                ...
                out_label_np[index] = label

            img = mx.image.imdecode(img, to_rgb=1).asnumpy()
            out_img_np[index] = img

    def _workers(self, imgrec, out_img_np, out_label_np=None, read_label=False, num_workers=1):
        import multiprocessing as mp
        import threading
        record = []
        for worker in range(num_workers):
            lo = self.total // num_workers * worker
            hi = self.total // num_workers * (worker + 1)
            hi = min(hi, self.total)
            process = mp.Process(target=self._all_unpack, args=(imgrec, out_img_np, lo, hi,
                                                                out_label_np, read_label))
            process.start()
    ...
    def __getitem__(self, index):
        ...
        label, img = self.train_label_np[index], self.train_np[index]
        ...
        if not mask_flag:
            ...
            # header, image = mx.recordio.unpack(s1)
            # img_masked = mx.image.imdecode(image, to_rgb=1).asnumpy()  # masked face, rgb
            img_masked = self.mask_out_np[index]

            # s2 = self.mask_rec.read_idx(idx)
            # header, image = mx.recordio.unpack(s2)
            # mask = mx.image.imdecode(image, to_rgb=1).asnumpy()  # mask, rgb
            mask = self.mask_np[index]
