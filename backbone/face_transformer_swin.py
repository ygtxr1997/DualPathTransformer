...
from backbone.face_transformer import IResBackbone, IBasicBlock
...
class FaceSwinTransformerBackbone(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self,
                 img_size=112,
                 patch_size=4, in_chans=3, num_classes=512,
                 embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 window_size=7, mlp_ratio=4., qkv_bias=True,
        ...
        # self.avgpool = nn.AdaptiveAvgPool1d(1)
        # self.head = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
        fc = nn.Linear(self.num_features * 7 * 7, num_classes)  # TODO: fix feature size to 512
        features = nn.BatchNorm1d(num_classes, eps=1e-05)
        nn.init.constant_(features.weight, 1.0)
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)  # B L C
        x = x.transpose(1, 2)  # B C L, (B,768,49)
        x = torch.flatten(x, 1)  # (B,37632)
        return x
...
class FaceSwinTransformerWithHeader(nn.Module):
    def __init__(self,
                 header_type: str,
        ...
        super(FaceSwinTransformerWithHeader, self).__init__()
...
if __name__ == "__main__":
    ...
    w = 112
    d = 64
    # feat = torch.randn(5, d, h, w)
    # layer = PatchEmbed(h, patch_size=2, in_chans=d, )

    # feat = torch.randn(1, h * w, d)
    # layer = PatchMerging(input_resolution=[h, w], dim=d)
    #
    # output = layer(feat)
    # print(output.shape)
    # exit()
    ...
    with torch.no_grad():
        ...
        config_file = 'configs/test_fst40m.yaml'
        # config_file = 'configs/train_fst.yaml'
        config = OmegaConf.load(config_file)
        net = instantiate_from_config(config.model).cuda()
        net.eval()
        print('Model loaded from %s.' % config_file)
